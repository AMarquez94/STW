...
function showAllMemo(rows, callback){
    ...
    for (var i = 0; i < rows.length; i++) {
        body = body +
                '<tr>'+
                '<td><a href="showMemo?id=' + rows[i]._id + '">' + rows[i].texto + '</a></td>' +
                '<td>' + rows[i].fecha + '</td>';
        if(rows[i].fichero == null){
            ...
            body = body + '<td><a href="downloadFile?id=' + rows[i]._id + '">' +
                rows[i].nombreFichero + '</a></td>'
        }
        body = body + '<td><a href="deleteMemo?id=' + rows[i]._id + '">Eliminar tarea</a></td></tr>';
    };
...
function showMemo(row, callback){
    var body = '<html>'+
        ...
        '<td style="width:80%">' + row.texto + '</td>' +
        '</tr>'+
        '<tr>' +
        '<td style="width:20%"><b>Fecha límite</b></td>' +
        '<td style="width:80%">' + row.fecha + '</td>' +
        '</tr>'+
        '<tr>' +
    ...
    if(row.fichero == null){
        ...
        body = body + '<td style="width:80%"><a href="downloadFile?id=' +
            row._id + '">' + row.nombreFichero + '</a></td>';
    }
        body = body + '</tr>'+
        '</table>' +
        '<p><a href="start">Volver al inicio</a></p>' +
